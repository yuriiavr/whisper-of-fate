...
const MagicalCard = ({ card, isReversed, index }: { card: TarotCard; isReversed: boolean; index: number }) => {
  ...
  return (
    <div className={`flip-card h-[400px] w-[240px] relative ${isFlipped ? "flipped" : ""}`}>
      <div className="flip-card-inner w-full h-full shadow-2xl">
        <div className="flip-card-front bg-magical-depth border-2 border-magical-accent p-1.5 flex items-center justify-center overflow-hidden">
          <img src={CARD_BACK_URL} alt="Card Back" className="w-full h-full object-cover rounded-md" />
        </div>
        <div className="flip-card-back bg-magical-depth border border-gray-800 p-3 flex flex-col items-center overflow-hidden">
          <div className="relative w-full h-full rounded-lg overflow-hidden">
            ...
            <div className="absolute bottom-3 left-3 right-3 text-center">
              <p className="text-xl font-bold text-white uppercase">{card.name}</p>
              <p className="text-[10px] text-magical-gold uppercase opacity-80">{card.nameEn}</p>
              {isReversed && <p className="text-[10px] text-red-400 font-bold mt-1">(ПЕРЕВЕРНУТА)</p>}
            </div>
          </div>
        </div>
      </div>
...
export default function TarotPage() {
  ...
  const handleDivine = async () => {
    ...
    try {
      ...
      setInterpretation("Вибачте, духи не змогли дати відповідь. Спробуйте пізніше.");
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <section className="bg-magical-depth/60 border border-gray-800 p-8 rounded-3xl shadow-2xl mb-16 backdrop-blur-sm">
        <div className="flex flex-col gap-6">
          <div className="w-full">
            <label className="block text-xs font-semibold text-gray-500 tracking-widest uppercase mb-2">Ваш запит</label>
            <input
              type="text"
              value={query}
          ...
          <div className="flex flex-wrap gap-4 justify-center">
            ...
            <button 
              ...
              className="px-6 py-3 bg-gray-800 hover:bg-gray-700 rounded-xl text-white font-semibold transition-all active:scale-95 border border-gray-700 flex items-center gap-2"
            >
              🎴 {drawnCards.length === 3 ? "Новий розклад" : "3 карти (Минуле/Теперішнє/Майбутнє)"}
            </button>
          </div>
        </div>
      </section>
        ...
        <section className="mb-16">
          <div className="flex justify-center gap-6 md:gap-10 flex-wrap mb-12">
            {drawnCards.map((drawn, index) => (
              <MagicalCard key={`${drawn.card.id}-${index}`} card={drawn.card} isReversed={drawn.isReversed} index={index} />
            ))}
          </div>
