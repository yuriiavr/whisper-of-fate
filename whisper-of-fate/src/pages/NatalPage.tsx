  ...
  return (
    <div className="flex justify-center my-8">
      <svg width="300" height="300" viewBox="0 0 300 300" className="drop-shadow-2xl">
        <circle cx={centerX} cy={centerY} r={radius} fill="none" stroke="#4c1d95" strokeWidth="2" />
        <circle cx={centerX} cy={centerY} r={radius - 20} fill="none" stroke="#1e1b4b" strokeWidth="1" opacity="0.5" />
        ...
        {planets?.map((p, i) => {
          ...
          return (
            <g key={i}>
              <line x1={centerX} y1={centerY} x2={x} y2={y} stroke="#8b5cf6" strokeWidth="0.5" opacity="0.3" />
              <circle cx={x} cy={y} r="5" fill="#d946ef" className="animate-pulse" />
              <text x={x + 8} y={y + 4} fontSize="10" fill="#ddd" className="font-mono select-none">
...
export default function AstroPage() {
  ...
  const [p1, setP1] = useState<PersonData>({ name: "Я", date: "", time: "12:00", city: "", coords: null });
  const [p2, setP2] = useState<PersonData>({ name: "Партнер", date: "", time: "12:00", city: "", coords: null });
    ...
    <div className="max-w-4xl mx-auto p-4 animate-in fade-in duration-700">
      <div className="flex gap-4 mb-8 justify-center">
        <button 
          ...
          className={`px-6 py-2 rounded-full transition font-medium ${mode === 'natal' ? 'bg-magical-accent text-white shadow-lg shadow-purple-500/20' : 'bg-gray-800 text-gray-400 hover:text-gray-200'}`}
        >
          Натальна карта
        </button>
        <button 
          onClick={() => { setMode("synastry"); setResult(null); }}
          className={`px-6 py-2 rounded-full transition font-medium ${mode === 'synastry' ? 'bg-magical-accent text-white shadow-lg shadow-purple-500/20' : 'bg-gray-800 text-gray-400 hover:text-gray-200'}`}
        >
      ...
      <div className="bg-magical-depth/40 border border-gray-800 p-8 rounded-3xl backdrop-blur-md shadow-2xl mb-12">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          <div className="space-y-4">
            ...
            <input 
              type="date" 
              className="w-full p-3 bg-magical-dark border border-gray-700 rounded-xl text-white focus:border-magical-accent outline-none transition"
              onChange={(e) => setP1({...p1, date: e.target.value})} 
            />
            <input 
              type="time" 
              className="w-full p-3 bg-magical-dark border border-gray-800 rounded-xl text-white focus:border-magical-accent outline-none transition"
              value={p1.time}
            ...
            <div className="relative">
              ...
              {cities.target === "p1" && cities.list.length > 0 && (
                <div className="absolute z-50 w-full bg-gray-900 border border-gray-700 rounded-xl mt-1 overflow-hidden shadow-2xl">
                  {cities.list.map((c, i) => (
                    <div key={i} className="p-3 hover:bg-magical-accent cursor-pointer text-sm text-gray-200" onClick={() => {
          ...
          {mode === "synastry" && (
            <div className="space-y-4 animate-in slide-in-from-right duration-500">
              ...
              <input 
                type="date" 
                className="w-full p-3 bg-magical-dark border border-gray-700 rounded-xl text-white focus:border-magical-accent outline-none"
        ...
        >
          ...
          ) : mode === "natal" ? "Розрахувати натал" : "Перевірити сумісність"}
        </button>
      </div>
      ...
      {result && (
        <div className="bg-magical-depth/60 p-8 rounded-3xl border border-gray-800 animate-in zoom-in duration-500">
          ...
          {mode === "natal" && result.planets && (
            <NatalVisual planets={result.planets} />
          )}
