...
export default async function handler(req, res) {
  ...
  try {
    if (type === "tarot") {
      ...
      const styleInstruction = isTrollMode 
        ? `Ти — зухвалий, саркастичний та надзвичайно прямолінійний таролог-цинік з чорним гумором. 
           Якщо запит абсурдний, вульгарний або тупий — відповідай у тому ж дусі (наприклад, карти з довгими предметами можуть символізувати щось інтимне, якщо контекст дозволяє). 
           Будь гострим на язик, використовуй сленг, сарказм та грубі метафори. Можна не бути толерантним.`
        : `Ти — досвідчений таролог та психолог. Твій тон мудрий, глибокий та етичний. 
           Давай корисні поради та допомагай користувачу знайти шлях.`;
    ...
    Твоє завдання:
    1. Проаналізуй кожну карту відповідно до її ПОЗИЦІЇ у розкладі.
    ...
    4. Якщо вибрано стібний режим — не стримуй себе в жартах.

    Відповідай українською мовою у форматі Markdown.

    ВАЖЛИВО: Ваша відповідь ОБОВ'ЯЗКОВО має починатися з короткої (до 10 слів) поради-цитати на день у відповідному стилі.
